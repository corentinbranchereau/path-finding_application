...
public class Planning {
    ...
    /**
     * Map (statique) permettant d'identifier chaque élément d'une requête à une
     * intersection géographique à partir de son identifiant unique
     */
    ...
    public void calculTempsDePassage() {
        ...
        for (Long id : tournee) {
            ...
            for (Requete r : requetes) {
                if (r.getDemandeLivraison().getIdDemande() == id) {
                    duree += r.getDemandeLivraison().getDuree() * 1000;
                    break;
                }

                if (r.getDemandeCollecte().getIdDemande() == id) {
                    duree += r.getDemandeCollecte().getDuree() * 1000;
                    break;
                }
    ...
    /**
     * Génère des ids uniques pour chaque point de collecte ou de livraison d'une
     * requête
     * 
     * @param planning
     */
    public void generateNewId() {
        ...
        for (Requete requete : requetes) {
            idUniqueTOIdIntersection.put(requete.getDemandeCollecte().getIdDemande(),
                    requete.getDemandeCollecte().getIdIntersection());
            // requete.setIdUniquePickup(id);
            id += 1;
            idUniqueTOIdIntersection.put(requete.getDemandeLivraison().getIdDemande(),
                    requete.getDemandeLivraison().getIdIntersection());
    ...
    /**
     * Permet de faire une modification de requête : changement d'un point de pickup
     * ou delivery
     * 
    ...
    public Planning modifierRequeteLieu(long idUnique, long idIntersection) {
        ...
        for (Requete r : requetes) {
            if (r.getDemandeLivraison().getIdDemande() == idUnique) {
                r.getDemandeLivraison().setIdIntersection(idIntersection);
            }

            if (r.getDemandeCollecte().getIdDemande() == idUnique) {
                r.getDemandeCollecte().setIdIntersection(idIntersection);
    ...
    public Planning modifierOrdreRequetes(long idUnique1, long idUnique2) {
        ...
        /*
         * for(Long l : this.idUniqueTOIdIntersection.keySet()) { System.out.println(l +
         * ":" +this.idUniqueTOIdIntersection.get(l)); }
         */
    ...
    public Planning ajouterRequete(Requete newRequete) {
        ...
        // maj de la map des correspondances d'ids
        idUniqueTOIdIntersection.put(idUniqueMax + 1, newRequete.getDemandeCollecte().getIdDemande());
        idUniqueTOIdIntersection.put(idUniqueMax + 2, newRequete.getDemandeLivraison().getIdDemande());
